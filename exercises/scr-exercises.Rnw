...
\item Create a plot of the detector locations. Their coordinates can
  be found in \texttt{test.data\$traps}.
...
save(par.ests, D.cis, g0.cis, sigma.cis, file = "sim-res.RData")
@   
...
## CI coverage for each parameter.
mean(D.cis[-1, 1] <= 0.75 & D.cis[-1, 2] >= 0.75)
mean(g0.cis[-1, 1] <= 0.9 & g0.cis[-1, 2] >= 0.9)
mean(sigma.cis[-1, 1] <= 75 & sigma.cis[-1, 2] >= 75)
@ 

\answer{The averages for all three parameters are close to the true
  parameter values. Our estimators appear more-or-less unbiased. With
  only 100 interations, it's hard to determine whether or not CI
  coverage reaches the nominal 95\%, but all three are more or less
  there. We'd get a better indication with a larger number of
  iterations.}
